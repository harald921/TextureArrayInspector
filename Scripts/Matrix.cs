...
namespace TextureArrayInspector 
{
	...
	public class Matrix : Matrix2<float>
	{
		...
		#region Overriding constructors and clone
			...
			public Matrix Copy (Matrix result=null) //TODO: obsolete
			{
		...
		#region Texture
			...
			public Texture2D ToTexture () { return ToTexture(rect.size.x, rect.size.z); }
			...
			///Filling texture as is, regardless of the matrix offset. If texture is larger than matrix remaining area will not be changed.
			...
			///Filling texture using both matrix and texture offsets. Filling only intersecting rect/texture are, leaving other unchanged.
			{
				...
				CoordRect intersect = CoordRect.Intersect(rect, textureRect);
		...
		#region Conversion
			...
			/// Fills matrix with array 2D pixel-to-pixel. If array size is smaller than matrix jutting values will not be filled. Using swapped X and Z.
			{
				Coord max = new Coord( Mathf.Min(array.GetLength(0), rect.size.x), Mathf.Min(array.GetLength(1), rect.size.z) );
				for (int x=0; x<max.x; x++)
					for (int z=0; z<max.z; z++)
						this.array[z*rect.size.x + x] = array[z,x]; //x and z swapped
			}

			public void ReadArrayResize (float[,] array)
			/// Virtually resizes array to matrix size and copies values. Using swapped X and Z. If size match using direct copy.
			{
				Coord arrSize = new Coord(array.GetLength(0), array.GetLength(1));
				if (arrSize.x==rect.size.x && arrSize.z==rect.size.z)
					{	ReadArrayDirect(array); return; }

				Coord min = rect.Min; Coord max = rect.Max;
				for (int x=min.x; x<max.x; x++)
					for (int z=min.z; z<max.z; z++)
				{
					double px = 1d*(x-rect.offset.x) / rect.size.x; //percent
					double pz = 1d*(z-rect.offset.z) / rect.size.z;

					float val = array.GetInterpolated((float)(px*arrSize.x), (float)(pz*arrSize.z));

					this.array[z*rect.size.x + x] = val;
				}
			}
